...
export default function PaymentMethodSelector({
  ...
  // Only display Apple Pay on compatible Apple devices (iOS, iPadOS, macOS Safari)
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && typeof navigator !== "undefined") {
      ...
      const supported = isAppleDevice && hasApplePay;
      setCanUseApplePay(supported);

      if (!supported && selectedMethod === "apple_pay") {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`grid grid-cols-1 ${canUseApplePay ? "sm:grid-cols-3" : "sm:grid-cols-2"} gap-3`}>
        {/* Stripe Card Option */}
        <button
          type="button"
          disabled={disabled}
        ...
        {/* Apple Pay Option (Only shown on supported Apple devices) */}
        {canUseApplePay && (
          <button
            type="button"
            disabled={disabled}
