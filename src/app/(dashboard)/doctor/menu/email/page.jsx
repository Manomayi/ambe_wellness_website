  ...
  return (
    <div className="max-w-lg mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-[#353535] mb-1">Current Email</label>
          <input
            type="email"
            value={currentEmail}
            disabled
            className="w-full p-2 border border-[#E7E2D9] bg-white text-black rounded-lg"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-[#353535] mb-1">New Email</label>
          <input
            type="email"
            value={newEmail}
            onChange={e => setNewEmail(e.target.value)}
            className="w-full p-2 border border-[#E7E2D9] bg-white text-black rounded-lg focus:outline-none focus:border-[#C8996A]"
          />
        </div>
        {requiresReauth && (
          <div>
            <label className="block text-sm font-medium text-[#353535] mb-1">Password</label>
            <input
              type="password"
              value={password}
              onChange={e => setPassword(e.target.value)}
              className="w-full p-2 border border-[#E7E2D9] text-black rounded-lg focus:outline-none focus:border-[#C8996A]"
            />
          </div>
        )}
        <button
