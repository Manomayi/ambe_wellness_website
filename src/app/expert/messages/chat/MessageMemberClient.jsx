...
export default function MessageMemberClient() {
  const router = useRouter()
  const params = useSearchParams()
  const chatId = params.get('chatId')
  const memberName = params.get('memberName')
  const memberPhotoUrl = params.get('memberPhotoUrl')
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex items-center p-4 bg-white shadow">
        ...
        {memberPhotoUrl && (
          <img src={memberPhotoUrl} alt={memberName||''}
               className="h-10 w-10 rounded-full mx-3 object-cover" />
        )}
        <h1 className="text-lg font-medium text-gray-800">{memberName}</h1>
      </div>
