...
export default function ExpertMemberProfilesPage() {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        const snap = await getDoc(
          doc(db, 'doctors', user.uid, 'patients', 'current')
        );
        const data = snap.exists() ? snap.data() : {};
        setMembers(Array.isArray(data.patients) ? data.patients : []);
      } catch (e) {
  ...
  return (
    <div className="space-y-12">
      <h1 className="text-2xl font-semibold text-gray-800">Member Profiles</h1>
